...
public class DOTweenSceneManager : MonoBehaviour
{
    ...
    private void CleanupTrackedTransforms()
    {
        ...
        foreach (var t in toRemove) trackedTransforms.Remove(t);
    }
    ...
    {
        if (useAdvancedCleanup) StartCoroutine(AdvancedCleanupCoroutine());
        else KillAllDOTweenAnimations();
    }
    ...
    {
        if (pauseStatus) PauseAllDOTweenAnimations();
        else ResumeAllDOTweenAnimations();
    }

    void OnApplicationFocus(bool hasFocus)
    {
        if (!hasFocus) PauseAllDOTweenAnimations();
        else ResumeAllDOTweenAnimations();
    }
}
